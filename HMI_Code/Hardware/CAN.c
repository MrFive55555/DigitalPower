...
void CANInit(void){
	...
	CAN_FilterInitStructure.CAN_FilterIdHigh= ((((u32)0x1FFFFFFF<<3)|CAN_ID_EXT|CAN_RTR_DATA)&0xFFFF0000)>>16;
	//要筛选的 ID 低位
	CAN_FilterInitStructure.CAN_FilterIdLow= (((u32)0x1FFFFFFF<<3)|CAN_ID_EXT|CAN_RTR_DATA)&0xFFFF;
	//筛选器高 16 位每位必须匹配
	...
	while(*stringData!='\0'){
		...
		if(ubCounter>=8) {
			...
			while(CAN_TransmitStatus(CAN1,TxMailBox)!=CAN_TxStatus_Ok){
				//printf("len>8\r\n");
				if(CAN_TransmitStatus(CAN1, TxMailBox) == CAN_TxStatus_NoMailBox) {
					printf("No available mailbox\r\n");
					return;  // 如果没有可用的邮箱，退出
				}
...
void USB_LP_CAN1_RX0_IRQHandler(void){
	...
	if((RxMessage.ExtId!=0x1FFFFFFF)||(RxMessage.IDE!=CAN_ID_EXT)||RxMessage.DLC!=8){
