...
const Uint16 tempTable[] = { 3413, 3351, 3290, 3228, 3167, 3107, 3046, 2987,
                             2927, 2868, 2810, 2753, 2695, 2639, 2582, 2527,
                             2472, 2418, 2365, 2312, 2260, 2208, 2158, 2108,
                             2059, 2011, 1963, 1916, 1870, 1825, 1781, 1737,
                             1695, 1653, 1612, 1572, 1533, 1494, 1457, 1420,
                             1384, 1349, 1314, 1281, 1248, 1216, 1184, 1154,
                             1124, 1095, 1067, 1039, 1012, 986, 961, 936, 912,
                             888, 865, 843, 830, 800, 779, 760, 740, 721, 703,
                             685, 668, 650, 634, 618, 602, 588, 573, 567, }; //number:76 temperature table
...
Uint16 ADCConvertCalData(void)
{
    ...
    {
        elecData.outputVol =
                ((elecCalData.outputVol + calAdjust.outputVolAdjust) * 3
                        * CAL_EXPANSION_FACTOR) / ADC_VALUE_MAXIMUM;
        elecData.inputVol = ((elecCalData.inputVol + calAdjust.inputVolAdjust)
                * 3 * CAL_EXPANSION_FACTOR) / ADC_VALUE_MAXIMUM;
        //elecData.tempVol = ((elecCalData.tempVol + calAdjust.tempVolAdjust) * 3 * CAL_EXPANSION_FACTOR) / ADC_VALUE_MAXIMUM;
        elecData.current = ((elecCalData.current + calAdjust.currentAdjust) * 3
                * CAL_EXPANSION_FACTOR) / ADC_VALUE_MAXIMUM;
...
static void ADCGetTemp(void)
{
    ...
    while (max <= min)
    {
        ...
        {
            elecData.tempVol += 25; //from 25¡æ start count
            break;
        }
    }
    elecData.tempVol += 25;
...
static void ADCGetCurrent(void)
{
    elecData.current = (elecCalData.current - ZERO_CURRENT_VOL) * PER_CURRENT_VOL
            / 10; // unit: mA
}
static void ADCGetPower(void)
{
