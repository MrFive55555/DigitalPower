...
rt_uint8_t parameterTemp[STRING_BUFFER_MAX_LENGTH / 2];
...
{
	static rt_uint8_t head_queue_temp;
	static rt_uint16_t data_count_error;
	if (data_count <= 0)
	{
	...
	else
	{
		...
		if (data_count != 0)
		{
			...
			{
				data_count_error = (STRING_BUFFER_MAX_LENGTH - head_queue_temp + head_queue);
			}
			else
			{
				data_count_error = (head_queue - head_queue_temp);
			}
			if (data_count_error <= data_count)
				data_count -= data_count_error;
			else
				data_count -= data_count;
		}
	}
}
rt_uint32_t crc32CodeCal;
static void usartReceiveElecData(void)
{
	rt_uint8_t index = 0;
	rt_uint8_t i;

	while (string_buffer[head_queue] != CHAR_ENTER)
	{
		// if (string_buffer[head_queue] != CHAR_BLANK && string_buffer[head_queue] != CHAR_ENTER && string_buffer[head_queue] != '.' && string_buffer[head_queue] != '\0')
		// {
	...
	// crc32 check
	crc32Code = parameterTemp[index - 4] << 24 | parameterTemp[index - 3] << 16 | parameterTemp[index - 2] << 8 | parameterTemp[index - 1];
	crc32CodeCal = crc32(parameterTemp, index - 4);
	if (crc32CodeCal != crc32Code)
	{
