...
int main(void)
{
	...
	/*while(1){
		//CAN_SendMsg(sendData);
		//printf("loop.\r\n");
		//TIM_SetAutoreload(TIM1,11999);
		//TIM_SetCompare1(TIM1,5999);
...
static void create_thread(void *parameter)
{
	...
	// creater timer for uart receive data
	timer_uart_receive_1s = rt_timer_create("timer1", timer_uart_receive_data, RT_NULL, 100, RT_TIMER_FLAG_PERIODIC | RT_TIMER_FLAG_SOFT_TIMER);
