...
static rt_uint8_t parameterTemp[4 * ELEC_DATA_LENGTH];
...
void USART1_IRQHandler(void)
{
	if (USART_GetITStatus(USART1, USART_IT_RXNE) != RESET)
	{
		...
		else
		{
			...
			if (tail_queue < STRING_BUFFER_MAX_LENGTH-1) tail_queue++;
			else tail_queue = 0;
			data_count++;
		}
	}
...
{
	rt_uint8_t head_queue_first;
	if (data_count <= 0 && head_queue == tail_queue)
	{
		//printf("circle queue of data is empty.\r\n");
	}
	else
	{
		head_queue_first = head_queue;
		if (string_buffer[head_queue++] == 'P') // P indicates get parameters
		{
		...
		else if (string_buffer[0] == 'S') // S indicates setting command
		{
		}
		else
		{
		...
		if (head_queue <= head_queue_first)
		{
			data_count -= (STRING_BUFFER_MAX_LENGTH - head_queue_first + head_queue);
		}
		else
		{
			data_count -= (head_queue - head_queue_first);
		}
	}
}
static void usartReceiveElecData(void)
{
	...
	for (; string_buffer[head_queue] != '\0';)
	{
